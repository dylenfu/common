...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		d = td.Delay()
		t.signal.Reset(d)
		fmt.Printf("timer: add reset delay %d ms", int64(d)/int64(time.Millisecond))
	}
	fmt.Printf("timer: push item key: %s, expire: %s, index: %d\r\n", td.Key, td.ExpireString(), td.index)
}
...
func (t *Timer) del(td *TimerData) {
	...
	if i < 0 || i > last || t.timers[i] != td {
		...
		fmt.Printf("timer del i: %d, last: %d, %p\r\n", i, last, td)
		return
	}
	if i != last {
		t.swap(i, last)
